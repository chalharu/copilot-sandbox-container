...
実行対象は `hooks/postToolUse/linters.json` で定義します。この JSON は `tools` と `pipelines` の 2 セクションだけを持ちます。`tools` は実行コマンドと引数を含む concrete な定義で、必要に応じて対象ファイルを引数に付けないコマンドも表現できます。`pipelines` は regex matcher 配列と実行順、各 step の fallback tools をまとめて表します。
