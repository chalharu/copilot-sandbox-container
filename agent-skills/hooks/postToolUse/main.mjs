...
function loadLintersConfig() {
  const raw = fs.readFileSync(new URL('./linters.json', import.meta.url), 'utf8');
  ...
  for (const pipeline of parsed.pipelines ?? []) {
    ...
    if (!Array.isArray(pipeline.steps) || pipeline.steps.length === 0) {
      throw new Error(`Pipeline "${pipeline.id}" must define at least one step.`);
    }

    for (const step of pipeline.steps) {
      if (!Array.isArray(step.tools) || step.tools.length === 0) {
        throw new Error(`Each step in pipeline "${pipeline.id}" must define at least one tool.`);
      }

      for (const toolId of step.tools) {
        if (!tools.has(toolId)) {
          throw new Error(`Unknown tool referenced by pipeline "${pipeline.id}": ${toolId}`);
        }
      }
    }
...
function compileMatcher(pipelineId, matcher) {
  if (!Array.isArray(matcher) || matcher.length === 0) {
    throw new Error(`Pipeline "${pipelineId}" must define matcher as a non-empty array.`);
  }

  if (matcher.some((pattern) => typeof pattern !== 'string' || pattern === '')) {
    throw new Error(`Pipeline "${pipelineId}" matcher entries must be non-empty regex strings.`);
  }
...
function classifyFilesByPipeline(repoRoot, files) {
  const filesByPipeline = new Map(config.pipelines.map((pipeline) => [pipeline.id, []]));
  const matchedFiles = [];
...
async function main() {
  ...
  const changedFiles = getChangedFiles(repoRoot, currentRelevantFiles.matchedFiles, previousSignatures);
  ...
  process.exitCode = runPipelines(repoRoot, classifyFilesByPipeline(repoRoot, changedFiles).filesByPipeline);
  saveState(stateFilePath, repoRoot, getCurrentRelevantFiles(repoRoot).matchedFiles);
}
