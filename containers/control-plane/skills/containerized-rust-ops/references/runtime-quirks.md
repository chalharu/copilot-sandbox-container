...
- The sample bootstrap logic now lives in `containers/garage-bootstrap/` and runs as a one-shot bootstrap Job instead of a sidecar on every Garage pod start.
...
- The standalone Garage Deployment keeps the shared cache bucket under `/var/lib/garage` on the dedicated RWO PVC and mounts `/etc/garage/garage.toml` from the init-generated config volume, while the one-shot bootstrap Job talks to `garage-admin` / `garage-s3` Services.
