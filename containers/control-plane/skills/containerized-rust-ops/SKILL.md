...
The Kubernetes helper keeps the clone, temp files, `target`, and `sccache` state under `/var/tmp/containerized-rust/...`, while `cargo` and `rustup` stay under `/workspace/cache/...`. When the runtime exposes `SCCACHE_BUCKET`, `SCCACHE_ENDPOINT`, and S3 credentials through `AWS_ACCESS_KEY_ID_FILE` / `AWS_SECRET_ACCESS_KEY_FILE` (or direct env values), `k8s-rust.sh` writes an `SCCACHE_CONF` for the S3 backend. The sample manifest serves those requests from a standalone Garage Deployment backed by the dedicated `containers/garage/` helper image. Without object-store mode, the helper still uses the same ephemeral `sccache` path so `/workspace` does not accumulate object-cache data.
...
- `containers/garage/Dockerfile` for the sample in-cluster S3 backend image
