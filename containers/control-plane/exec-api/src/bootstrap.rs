...
pub(crate) fn prepare_server_environment(config: &ServerConfig) -> Result<(), DynError> {
    let Some(chroot_root) = config.chroot_root.as_deref() else {
        if can_prepare_direct_remote_home(config) {
...
#[cfg(unix)]
fn can_prepare_direct_remote_home(config: &ServerConfig) -> bool {
    config.mode == ServerMode::Exec && unsafe { libc::geteuid() == 0 }
}

#[cfg(not(unix))]
fn can_prepare_direct_remote_home(_config: &ServerConfig) -> bool {
    false
}
