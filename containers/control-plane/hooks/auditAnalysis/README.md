...
`postToolUse` のたびに監査ログ SQLite DB を読み、`~/.copilot/session-state/audit/audit-analysis.db` へ異常パターンの集計結果を保存する bundled hook です。

## 役割

- `userPromptSubmitted` を元にしたユーザー指摘の検出
- `postToolUse` を元にしたエラー解決パターンの追跡
- `preToolUse` を元にした繰り返し処理の検出
