...
Control Plane entrypoint は bundled hook JSON を root-owned な `COPILOT_HOME/hooks/` から読み、互換用に `~/.copilot/hooks/` symlink も張ります。親の `~/.copilot/` は sticky directory として管理されるため、監査対象の hook symlink は Copilot user から差し替えできません。`hooks.json` は `COPILOT_HOME` を優先して `hooks/audit/main` を起動するため、repo ごとの `.github/hooks/` を置かなくても、共通の監査ログ hook を保護された path から利用できます。
