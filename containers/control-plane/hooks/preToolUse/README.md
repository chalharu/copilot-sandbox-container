...
bundled rule は `deny-rules.yaml` で管理します。schema は `commandRules` と `protectedEnvironments` の 2 本立てで、command 側は basename を先頭に置いた短い rule 配列だけを書けばよい形です。
