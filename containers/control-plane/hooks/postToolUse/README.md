...
Copilot CLI の bundled `postToolUse` hook 設定です。repo-local 設定も統合し、
対象ファイルごとに linter pipeline を切り替えます。
...
Job 起動や待機などの基盤側 failure や remote 側の tool unavailable は local 実行へ
fallback しますが、remote 側の lint failure 自体はそのまま失敗として返します。
...
- Rust: bundled `control-plane-rust.sh` helper で changed file を最寄りの
  crate manifest / workspace manifest に解決し、`fmt` / `clippy --fix` /
  `clippy -D warnings`
- Dockerfile: `hadolint`
...
候補 tool が単に未 install で全部見つからない場合でも hook 全体は失敗させず、stderr に
skip message を出してその step を飛ばします。
