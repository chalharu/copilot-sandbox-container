...
browser から web UI へ到達できるところまでを通します。最後に
`./scripts/test-k8s-job.sh` を使い、Kubernetes Job 経路まで確認します。
...
- `copilot-sandbox` namespace に `control-plane` / `control-plane-web` Pod が起動している
- shared session PVC と workspace PVC が `Bound` している
- `http://127.0.0.1:8080/` で web UI を開ける
...
この tutorial で既定名のまま導入する場合に触るのは、次の 6 ファイルです。
...
- `deploy/kubernetes/control-plane.example/base/deployment-control-plane-web.yaml`
...
自分の公開鍵へ差し替えます。Deployment は SSH Service を公開しませんが、
image 内の legacy shell/runtime 互換のためこの key は引き続き渡します。
...
- `control-plane` / `control-plane-web` Pod が `Running`
...
port-forward を動かしたまま、browser か `curl` で確認します。

```bash
curl -fsS http://127.0.0.1:8080/healthz
```

browser 向けの Service は `control-plane`、Copilot ACP runtime 向けの internal
Service は `control-plane-acp` です。通常は browser から ACP Service へ直接
つなぐ必要はありません。
