...
- workspace は runner と SSH/terminal の同時アクセスを許容する
...
- `workspace-ssh` Pod: 任意要求時だけ起動する SSH endpoint。`sshd` と browser terminal 用 `ssh-authorizer` sidecar を含む
...
- `switchStrategy`: `create-then-switch`
- `acpSessionId`
...
- `workspaceRef`
- `requestedBy`
- `authorizedKeyRef`: `authMode=user-key` のときだけ利用する saved key ID
- `serviceType`
...
stateDiagram-v2
  ...
  Running --> Error: ACP 失敗 / 永続化失敗 / execution cancel
...
- replacement session は先に `Pending` で作成し、`Idle` 到達後に `Workspace.status.activeSessionRef` を切り替えて旧 session を archive する
- replacement 作成失敗時は旧 session を維持し、workspace を無 session 状態へ落とさない
- `Running` の session は execution を cancel するまで archive 不可
...
パスワードハッシュは別の `user-<user-id>-password` Secret に `password-hash` key で保存する。controller は execution または SSH endpoint 作成時に、必要な key だけを workspace namespace の短命 Secret へ投影する。`authorized_keys` は canonical copy ではなく、選択した saved key または browser terminal 用 generated key から workspace namespace 側で生成する。
...
- session create / archive API
- browser terminal session 作成 API
- execution cancel API
- execution cancel API では対象 `PromptExecution.spec.cancelRequested=true` を記録する
...
- `UserAccount` 無効化時に login session 失効、execution cancel、SSH endpoint / browser terminal revoke を行う
- replacement session の `create-then-switch` を制御する
- `PromptExecution.spec.cancelRequested=true` を検知したら runner 停止と `Cancelled` 反映を行う
...
- `ClusterIP` の場合は `ssh-authorizer` sidecar が `BrowserTerminalSession` を watch し、生成した内部公開鍵を shared `emptyDir` の `authorized_keys.d/` へ materialize する
- `BrowserTerminalSession` は鍵の materialize 完了後にだけ `Ready` へ遷移させる
...
sequenceDiagram
  ...
  R-->>C: Job 完了
  C->>C: create-mode かつ replacement 時は activeSessionRef 切替と旧 session archive
...
sequenceDiagram
  ...
  U->>W: 実行中止
  W->>A: DELETE /executions/{executionId}
  A->>A: PromptExecution に cancelRequested=true を反映
  C->>C: cancelRequested を reconcile
  C->>R: stop signal / Job delete
  R-->>C: partial state flush
  C->>A: Execution=Cancelled, Session=Error
  A-->>W: 状態更新
```
...
sequenceDiagram
  ...
  alt LoadBalancer
    ...
    C->>A: browserTerminalSupported=true / expiresAt を反映
  end
  A-->>W: endpoint 表示
```

### 7.5 ClusterIP のブラウザ端末ログイン
...
sequenceDiagram
  ...
  participant P as ssh-authorizer
  ...
  C->>C: 一時 SSH 鍵を内部生成
  C->>P: generated public key を materialize 要求
  P->>C: key install 完了通知
  C->>A: websocketPath / expiresAt / sshReadyAt 反映
  W->>T: WebSocket 接続
  T->>S: 内部生成鍵で SSH 接続
  S-->>T: PTY stream
  T-->>W: terminal stream
...
- `POST /api/workspaces/{workspaceId}/sessions/{sessionId}/archive`
- `POST /api/workspaces/{workspaceId}/executions`
- `DELETE /api/workspaces/{workspaceId}/executions/{executionId}`
...
- `409 Conflict`: 同一 workspace で active execution が存在する、または invalid session transition
...
- user 無効化時は login cookie を失効し、進行中 execution、SSH endpoint、browser terminal を revoke する
...
- workspace-ssh Pod の NetworkPolicy では `orchestrator-terminal-gateway` からの TCP/22 ingress を明示的に許可する
...
- user 無効化時は `UserDisabled` を理由に execution を `Cancelled` へ遷移させ、active cookie を失効する
...
- browser terminal 用 generated key は projected Secret 更新ではなく、`ssh-authorizer` sidecar による local materialization で扱う
- `runtime.env` 相当の設定ファイルは共有 home 配下へ置かず、Job / Pod ごとに生成して隔離する

## 14. 実装メモ: モデル利用方針

- 実装、設計レビュー、コードレビュー、文書レビューには `GPT-5.4` や `Claude Opus/Sonnet 4.6` のような高機能モデルを使う
- `gpt-4.1` は、コンテナ内で実際に Copilot へ問い合わせる E2E テスト、session create / `resume` の実行テスト、PremiumRequest 消費を避けたい回帰テストで指定する実行 model として使う
- つまり `gpt-4.1` は「設計を考える model」ではなく、「Copilot 実行経路を安価に検証する model」という位置づけに固定する
