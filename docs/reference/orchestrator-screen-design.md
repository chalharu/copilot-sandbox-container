...
未認証ユーザーを OIDC Provider へ誘導し、認証済み session を確立する。
...
| Git Credential | 任意 | private repository 用 Secret 参照 |
...
- `新規 session を開始` は既存 session を `Archived` へ遷移させる確認ダイアログを経由する
...
| 項目 | 必須 | 説明 |
| --- | --- | --- |
| Authorized Public Key | 必須 | 利用者の公開鍵 |
