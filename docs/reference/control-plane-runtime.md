...
same-namespace / same-node の Execution Pod へ自動委譲します。Read 系 tool は
Execution Pod から一時 cache へ copy した内容を読み、Write 系 tool は
Execution Pod 内へ先に書き込んだうえで control-plane 側の安全な一時 file へ
差し替えます。operator や agent が `bash` tool からこの helper を直接呼ぶ想定はありません。
