...
`ghcr.io/chalharu/copilot-sandbox-container/exec-pod:<tag>` image で
`/usr/local/bin/control-plane-exec-api serve` を直接起動します。initContainer /
node-scoped environment PVC / chroot は使いません。workspace PVC は
`CONTROL_PLANE_WORKSPACE_MOUNT_PATH`（既定 `/workspace`）へ直接 mount し、`gh` と
SSH の shared session mount も remote HOME (`/root`) 配下へ直接 mount します。
...
追加の Kubernetes volume / volumeMount は
`CONTROL_PLANE_FAST_EXECUTION_EXTRA_VOLUMES_JSON` と
`CONTROL_PLANE_FAST_EXECUTION_EXTRA_VOLUME_MOUNTS_JSON` に core/v1 `Volume` /
`VolumeMount` の JSON array として渡します。runtime は JSON array であること、
volume 名が built-in (`workspace`, `copilot-session`) と重複しないこと、mount が
既知 volume を参照すること、`mountPath` が絶対 path であることを検証します。
