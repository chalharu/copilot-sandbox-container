...
- custom Copilot provider 用の `COPILOT_PROVIDER_TYPE` / `COPILOT_PROVIDER_BASE_URL` / `COPILOT_MODEL`
...
- `copilot-provider-api-key`: custom provider の API key を startup 時に注入する場合だけ保持
...
`COPILOT_PROVIDER_API_KEY_FILE` も private runtime token file へ移します。
...
含めます。`node` / `pnpm` / `rustfmt` / `clippy` / `trunk` / `wasm-opt` /
`wasm-bindgen` / `cargo-deny` も入ります。`cargo-audit` / `taplo` / `mold` /
`lizard` と bundled hook も含まれます。remote HOME には `.gitconfig` を毎回生成します。
Rust/GTK 用の GTK4 / Pango development package と Rust/Wasm 用の
