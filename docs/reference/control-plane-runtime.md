...
値は core/v1 `Volume` / `VolumeMount` の JSON array です。runtime は JSON array
であることを検証します。volume 名は built-in (`workspace`, `copilot-session`) と
重複できません。mount は既知 volume を参照し、`mountPath` は絶対 path にします。
