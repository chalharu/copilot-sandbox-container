...
2. 再現性を重視するなら GitHub Packages の
   ...
   `control-plane-instance-env` 側の `CONTROL_PLANE_JOB_TRANSFER_IMAGE` も同じ
   tag へそろえる
3. `CONTROL_PLANE_FAST_EXECUTION_IMAGE` の sample 既定は
   `ghcr.io/chalharu/copilot-sandbox-container/exec-pod:latest`。shared cluster
   では digest-pinned ref を使う
...
10. `CONTROL_PLANE_FAST_EXECUTION_IMAGE` には dedicated exec-pod image を置く。
    ...
    hook、`git` / `gh` / `kubectl` / `sudo` / `sccache` を含め、本番では
    digest-pinned ref を使う。
    ...
    追加の cache や設定 file は
    `CONTROL_PLANE_FAST_EXECUTION_EXTRA_VOLUMES_JSON` /
    `CONTROL_PLANE_FAST_EXECUTION_EXTRA_VOLUME_MOUNTS_JSON` に Kubernetes
    `Volume` / `VolumeMount` JSON array として定義する。
    各 exec-pod 起動時に追加セットアップを走らせたい場合は
...
2. `~/.copilot/tmp` と Podman cache、fast-exec Execution Pod の
   `/var/tmp/control-plane` は generic ephemeral volume へ逃がし、再生成可能な cache が
   session PVC を食い潰さないようにする。
   storage class と合計サイズは generic volume JSON の
   `volumeClaimTemplate.spec` で調整する
...
4. `control-plane-workspace-pvc` も PVC spec である。storage class / サイズを
   ...
   exec-pod generic volume JSON の `storageClassName` も同時に見直す。
5. Rust Job の `cargo` / `rustup` / `target` などの再生成可能な state は
   `/var/tmp/containerized-rust/...` に寄せ、shared `/workspace` PVC に cache を
   溜めない。fast-exec Execution Pod でも image default または mounted
   `/root/.cargo/config.toml` で
   `target-dir = "/var/tmp/control-plane/cargo-target"` に固定する

exec-pod の Rust 設定を ConfigMap で上書きしたい場合は、`subPath` 付き mount を
generic volume JSON に追加します。たとえば sccache を明示する
`.cargo/config.toml` を配る場合は次のようにします。
...
data:
  CONTROL_PLANE_FAST_EXECUTION_EXTRA_VOLUMES_JSON: |
    [
      ...
      {
        "name": "cargo-config",
        "configMap": { "name": "exec-pod-cargo-config" }
      }
    ]
  CONTROL_PLANE_FAST_EXECUTION_EXTRA_VOLUME_MOUNTS_JSON: |
    [
      ...
      {
        "name": "cargo-config",
        "mountPath": "/root/.cargo/config.toml",
        "subPath": "config.toml",
        "readOnly": true
      }
    ]
