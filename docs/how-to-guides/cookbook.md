...
5. 永続化は RWX の copilot session PVC と RWO の `/workspace` PVC を基本にし、前者へ `~/.copilot/config.json`、`~/.copilot/command-history-state.json`、`~/.copilot/session-state`、`~/.copilot/session-state/audit/audit-log.db`、`~/.copilot/session-state/audit/audit-analysis.db`、`~/.config/gh`、`~/.ssh`、SSH host key をまとめる。`~/.copilot/tmp` と `/var/lib/control-plane/rootful-podman` は emptyDir の ephemeral storage に置き、sample manifest の rootful-service は `overlay` を既定にする
