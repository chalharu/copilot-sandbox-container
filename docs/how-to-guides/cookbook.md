...
1. 永続化は、初回導入時だけ `deploy/kubernetes/control-plane.example/install/shared-persistent-volume-claims.yaml`
...
3. shared PVC の spec は bound 後に自由に変更できないため、
   `control-plane.example/install/shared-persistent-volume-claims.yaml` の
   `control-plane-copilot-session-pvc` と `control-plane-sccache-pvc` は
