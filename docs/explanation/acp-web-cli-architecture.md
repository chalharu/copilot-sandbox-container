...
flowchart LR
    ...
    subgraph infrastructure["Infrastructure Layer"]
        ...
        permissionBroker["PermissionBroker<br/>ACP permission <-> UI decision"]
        workspaceFs["WorkspaceFileAdapter<br/>fs/read_text_file / fs/write_text_file"]
        terminalBroker["TerminalBroker<br/>terminal/create / output / wait / kill / release"]
    ...
    class router,httpHandlers,sseHandlers,sessionUseCases,conversationUseCases,permissionUseCases,completionUseCases,sessionModel,conversationModel,completionModel,supervisor,tcpClient,normalizer,permissionBroker,workspaceFs,terminalBroker,wasmAssets layer;
...
sequenceDiagram
    ...
    Backend->>SessionRepo: 仮 session metadata を作成
    ...
    Backend->>Worker: initialize(protocolVersion, clientCapabilities, clientInfo)
    ...
    Frontend->>Backend: GET /api/v1/sessions/{id}/events
...
sequenceDiagram
    ...
    Frontend->>Backend: POST /api/v1/sessions/{id}/messages
    ...
    opt tool 実行前に permission が必要
        ...
        Frontend->>Backend: POST /api/v1/sessions/{id}/permissions/{requestId}
...
sequenceDiagram
    ...
    Frontend->>Backend: GET /api/v1/sessions/{id}/events?after=last_sequence
...
sequenceDiagram
    ...
    Frontend->>Backend: GET /api/v1/completions/slash?sessionId=...&prefix=/co
...
sequenceDiagram
    ...
    alt ユーザーが明示 close
        Frontend->>Backend: POST /api/v1/sessions/{id}/close
...
sequenceDiagram
    ...
    alt file read / write
        ...
        Backend->>WorkspaceFS: absolute path で read / write
        WorkspaceFS-->>Backend: content / ok
        Backend-->>Worker: ACP client method result
    else command execution
        Worker->>Backend: terminal/create
        Backend->>TerminalBroker: command を起動
        TerminalBroker-->>Backend: terminalId
        Backend-->>Worker: terminalId
        Worker->>Backend: terminal/output or terminal/wait_for_exit or terminal/release
        Backend->>TerminalBroker: output / wait / release を実行
...
`clientCapabilities` は backend が**本当に提供できる ACP client methods**と一致させます。
初期設計では controlled runtime 前提で `fs.readTextFile=true`,
`fs.writeTextFile=true`, `terminal=true` を基本値とし、未実装の capability は `false`
で広告します。`session/request_permission` は ACP client の baseline method として常に
処理対象です。
...
- `CreateSession`
  - session ID 発行、初期状態作成、AcpWorker 起動、`initialize` / 必要なら
...
- `ResolvePermissionRequest`
  - pending permission request に対する user / policy の判断を受け取り、
    ACP の `session/request_permission` 応答へ写像する
- `CloseSession`
...
- `AcpTcpClient`
  - ACP over TCP の接続、JSON-RPC message serialize/deserialize、
    `initialize` / `authenticate` / `session/new` / `session/load` /
    `session/prompt` / `session/cancel` / `session/set_mode` の制御を行う
  - agent からの `session/request_permission`, `fs/*`, `terminal/*` 呼び出しを
    backend 内部 service へ dispatch する
- `EventNormalizer`
  - ACP の `session/update` 通知を `SessionEvent` や `ConversationItem` へ正規化する
  - `initialize` / `session/new` / `session/load` / `session/set_mode` の response から
    session metadata を更新する
  - delta の束ね、tool event の整形、mode 更新の反映、sequence 採番対象の決定を受け持つ
- `PermissionBroker`
  - `session/request_permission` を pending state と UI 応答待ちに変換する
  - cancel 時には ACP 規約どおり `cancelled` を返せるようにする
- `WorkspaceFileAdapter`
  - `fs/read_text_file` / `fs/write_text_file` を workspace 境界付きで実装する
- `TerminalBroker`
  - `terminal/create` / `output` / `wait_for_exit` / `kill` / `release` を実装する
- `SessionRepository`
  - session metadata、ACP sessionId、negotiated protocol/capabilities、TTL、
    worker ひも付け、attach 数などの現在値を保存する
- `EventStore`
...
| `POST` | `/api/v1/sessions/{id}/permissions/{requestId}` | permission 要求への応答 |
...
- permission auto-approval policy の既定値
