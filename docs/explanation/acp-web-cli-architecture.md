...
C4 図は「何があるか」を示す静的な図です。上司向けに説明する際は、下の表の順で
話すと、**境界 → 構成要素 → 動き → 障害時の戻り方**の順で理解してもらいやすくなります。
...
| 4.7 シーケンス図 | 実際の利用フローはどう流れるか | session 作成、会話実行、再接続、補完、終了の 5 シナリオ |
...
この図は、「ユーザーが会話を始めると backend が worker を起動し、SSE で初回状態を
配るまで」を示します。
...
sequenceDiagram
    ...
    Worker-->>Supervisor: ACP ready
    Supervisor-->>Backend: worker ready
    Backend->>SessionRepo: session と worker の対応を保存
...
主経路です。上司向けには、**上りは HTTP、下りは SSE** の分離を見る図です。
...
sequenceDiagram
    ...
    Backend->>Worker: ACP request を送信

    loop assistant / tool event が続く間
        Worker-->>Backend: raw ACP event
...
この図は、「SSE が切れたあとに同じ session へ戻る流れ」を示します。上司向けには、
**切断時も backend が state を持つので会話が壊れにくい**ことを説明する図です。
...
sequenceDiagram
    ...
    Frontend->>Backend: GET /api/v1/completions/slash?prefix=/co
    Backend->>CompletionUC: 候補解決を依頼

    opt session 依存の候補が必要
        CompletionUC->>Worker: ACP completion query
        Worker-->>CompletionUC: dynamic candidates
    end

    CompletionUC-->>Backend: static + dynamic 候補を正規化
...
示します。上司向けには、**どこで resources が増え、どこで確実に掃除されるか**を見る図です。
