...
- RWO の `/workspace` PVC
- RWO の dedicated `sccache-dist` PVC（5Gi）
...
standalone `sccache-dist` Deployment へ Service 越しに送ります。shared cache 本体は
...
ファイル向けの headroom を残します。dist mode を切った場合も、Job の
`sccache` 自体は `/var/tmp/containerized-rust/<repo>/<branch>/sccache` の
ephemeral path を使い、`/workspace` 側には `cargo` / `rustup` cache だけを残します。
