...
- **Control Plane** は Copilot ACP runtime、browser-facing web UI、`kubectl`、bundled hook/runtime を持つ長寿命の操作面
...
## 2. `kubectl exec` ではなく web UI + ACP を主経路にする理由

Control Plane を Kubernetes Pod で動かす場合、`kubectl exec` は保守用には便利ですが、
operator 体験としては扱いづらく、対話 state も直接露出しがちです。このリポジトリでは
browser-facing web UI と internal ACP runtime を分け、外向きの入口を HTTP に寄せます。

sample manifest の `control-plane` Service は web UI を、`control-plane-acp` Service は
web backend からだけ使う ACP endpoint を担います。これにより `kubectl exec` や
NodePort/LoadBalancer 上の SSH を operator の主経路から外しつつ、Copilot 側は
正式な ACP server surface に寄せられます。
...
copilot session PVC には `~/.copilot/config.json`、`~/.copilot/command-history-state.json`、`~/.copilot/session-state`、`~/.copilot/session-state/audit/audit-log.db`、`~/.config/gh`、`~/.config/control-plane/ssh-auth/authorized_keys`、`~/.ssh`、`/var/lib/control-plane/ssh-host-keys` をまとめます。これで Copilot session、GitHub 認証、Copilot の設定、監査ログに加え、legacy SSH compatibility 用の state も Pod 再作成後に残せます。sample manifest は SSH Service を公開しませんが、image 内の current-cluster smoke や互換 shell/runtime のために関連 path 自体は残します。
...
## 6. なぜ Job の `--mount-file` は web backend 経由の HTTP transfer なのか

ConfigMap 経由の file handoff は簡単ですが、サイズ制限が厳しく、大きめの入力ファイルや Job からの write-back を扱いにくいです。そのため Kubernetes Job path の `--mount-file` は、separate web deployment が持つ HTTP endpoint へ tar archive を出し入れする構成にしています。init container が入力 tar を pull し、sidecar が Job 完了後に出力 tar を push し、その後 `control-plane-job-transfer` が競合検知つき write-back を行います。
