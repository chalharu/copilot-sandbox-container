...
overlay を置きます。entrypoint は startup ごとに、それをもとに ephemeral な
`~/.copilot/config.json` を初期化します。これなら stale な PVC 上の旧 config や
コメント入り JSON に起動を引きずられません。Copilot 側の model 変更も runtime に
そのまま反映できます。
