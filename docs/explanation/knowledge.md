...
tool から直接呼ぶ想定はありません。この Pod は同じ `/workspace` PVC を
mount しつつ、同一ノードで共有する `/environment` RWO PVC 上の chroot runtime
を再利用します。session pod 自体は `sessionEnd` と OwnerReference の両方で
cleanup されますが、`/environment/root` 側はノード cache として残るため、
...
repo change delivery 系の bundled skill と bundled implementation / review agent
suite は image へ同梱し、起動時に `~/.copilot/skills/` / `~/.copilot/agents/`
へ同期します。
