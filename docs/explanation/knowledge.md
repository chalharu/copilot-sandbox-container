...
- **Control Plane** は Copilot CLI、ACP endpoint、`kubectl`、`gh`、SSH client state、bundled hook/runtime を持つ長寿命の操作面
...
## 2. `kubectl exec` ではなく ACP + web を主経路にする理由

Control Plane を Kubernetes Pod で動かす場合、`kubectl exec` は保守用には便利ですが、
browser 向け UI や自動化の入口としては扱いにくいです。このリポジトリでは
control-plane Pod の既定起動を `copilot --acp --port <port>` に寄せ、対話の公開面は
別 Deployment の Axum backend + Leptos CSR frontend から提供します。

こうしておくと、browser には HTTP API と軽量 UI だけを見せつつ、ACP の詳細や長い
応答処理は backend 側へ閉じ込められます。`kubectl exec` や明示的な `sshd` 起動は
保守用の escape hatch として残せても、通常運用の主経路にはしません。
...
copilot session PVC には `~/.copilot/config.json`、`~/.copilot/command-history-state.json`、`~/.copilot/session-state`、`~/.copilot/session-state/audit/audit-log.db`、`~/.config/gh`、`~/.config/control-plane/ssh-auth/authorized_keys`、`~/.ssh`、`/var/lib/control-plane/ssh-host-keys` をまとめます。これで Copilot session、GitHub 認証、SSH client state、Copilot の設定、監査ログは Pod 再作成後も残せます。`~/.config/control-plane/ssh-auth/authorized_keys` と host keys は、明示的に `sshd` を起動する保守経路や互換用途のために分離して残しており、通常の対話経路は ACP + web 側へ寄せます。`sshd` を使う場合も、private host key は PVC を直接参照せず、毎回 `/run/control-plane/ssh-host-keys` へ root-only copy を staging してから読むので、storage backend が group bit を残しても再起動しやすくなります。
...
## 6. なぜ Job の `--mount-file` は HTTP transfer endpoint なのか

ConfigMap 経由の file handoff は簡単ですが、サイズ制限が厳しく、大きめの入力ファイルや
Job からの write-back を扱いにくいです。そのため Kubernetes Job path の
`--mount-file` は、web backend が公開する HTTP endpoint を使って tar archive を
受け渡す構成へ切り替えています。init container が入力 tar を download し、Job
完了後は sidecar が出力 tar を upload します。
