...
copilot session PVC には `~/.copilot/config.json`、`~/.copilot/command-history-state.json`、`~/.copilot/session-state`、`~/.copilot/session-state/audit/audit-log.db`、`~/.config/gh`、`~/.ssh`、`/var/lib/control-plane/ssh-host-keys` をまとめます。これで Copilot session、GitHub 認証、SSH 鍵、Copilot の設定、監査ログは Pod 再作成後も残せます。
