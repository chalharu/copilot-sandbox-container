...
Pod 内ではなく outer runtime の制約です。rootless Podman を完全には保証できません。まず `cat /proc/self/uid_map` で Pod user namespace が効いているか確認してください。1 行目が `0 0 4294967295` のままなら `spec.hostUsers: false` が効いていません。そこが解消済みでも失敗する場合は outer runtime 側の user namespace 許可が必要です。`control-plane-run --mode k8s-job` か GitHub Actions / host runner を使ってください。背景説明は `README.md` の「Rootless Podman / nested runtime の扱い」も参照してください。
