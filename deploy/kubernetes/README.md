...
5. `control-plane.example/base/deployment-control-plane.yaml` /
     `control-plane.example/base/deployment-control-plane-web.yaml` または
...
workspace PVC 名 / ACP-web Service host / helper image のような instance 固有値を持つ
...
- `control-plane.example/control-plane/`
  - workspace PVC、web Service、ACP Service、2つの Deployment、instance 固有 env を
    まとめた control-plane entry point
...
`shared-resources/` と `control-plane/` を compose し、namespace / ACP-web Service host /
...
- `kubectl get pods -n copilot-sandbox` で `control-plane` / `control-plane-web` Pod が `Running`
- `kubectl port-forward service/control-plane 8080:8080 -n copilot-sandbox`
  の後に browser で `http://127.0.0.1:8080/` を開ける
...
`control-plane-exec-workloads` Role / RoleBinding も含みます。web UI 経由の
session や delegated `bash` から `kubectl -n copilot-sandbox-jobs ...` を使い、
