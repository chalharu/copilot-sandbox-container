...
4. `control-plane-auth` Secret に入れる認証情報
...
2. `control-plane.example/common/secret-control-plane-auth.yaml`
   - 必要な認証情報だけを差し替える
   - たとえば `gh-github-token` / `gh-hosts.yml` /
...
workspace PVC 名 / ACP host / web host / helper image のような instance 固有値を持つ
...
- `kubectl get pods -n copilot-sandbox` で `control-plane` と `control-plane-web` Pod が `Running`
- `kubectl port-forward service/control-plane-web 8080:8080 -n copilot-sandbox`
  の後に `curl http://127.0.0.1:8080/healthz` が `ok` を返す
- browser で `http://127.0.0.1:8080/` を開ける

sample 既定では、internal な ACP 用 `control-plane` Service と、
browser / API 向け `control-plane-web` Service の 2 面に分かれます。
また、delegated Execution Pod 用の `control-plane-exec`
ServiceAccount と、`copilot-sandbox-jobs` 側の
`control-plane-exec-workloads` Role / RoleBinding も含みます。delegated `bash`
から `kubectl -n copilot-sandbox-jobs ...` を使い、作業後に削除する前提の
Deployment / Service / Job / Pod を扱えます。
