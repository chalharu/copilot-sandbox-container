...
Rust / Wasm を使う場合、dedicated exec-pod image には `sccache` / `rustfmt` /
`clippy` / `trunk` / `wasm-opt` / `wasm-bindgen` を含めます。`cargo-deny` /
`cargo-audit` / `taplo` / `mold` / `node` / `pnpm` / `lizard` も使えます。
GTK4 / Pango の development package と `wasm32-unknown-unknown` target も同梱します。
