...
`copilot-sandbox-container` は、Copilot CLI 向けの Control Plane イメージ、用途別 Execution Plane の参照実装、Kubernetes 配備サンプル、検証スクリプトをまとめたリポジトリです。sample manifest の既定値では、Copilot CLI の `bash` tool を session 単位の Kubernetes Execution Pod へ自動委譲し、明示的な `control-plane-run` も Kubernetes Job 経由で実行します。
