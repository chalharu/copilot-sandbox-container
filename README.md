...
Docker Buildx が使える host か GitHub Actions を使ってください。Buildah を
個別に使いたい場合は `quay.io/buildah/stable` のような upstream イメージを
host / CI 側で利用してください。
...
`latest` と commit SHA に加えて、同梱ツールの version tag も更新します。
...
では `LANG=C.UTF-8` を補います。これにより、日本語を含む UTF-8 テキストも
SSH / GNU Screen 越しで表示しやすくしています。
