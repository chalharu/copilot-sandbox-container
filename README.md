...
Control Plane を提供します。起動時に Copilot / gh / SSH client state、
`runtime.env`、bundled hook / skill、`control-plane-run` を整え、control-plane Pod では
`copilot --acp --port <port>` を起動します。sample manifest では、別 Deployment として
Axum 製 backend と Leptos CSR frontend も立ち上げ、Copilot CLI の `bash` tool は
session-scoped Execution Pod へ自動委譲します。

## このコンテナイメージが行うこと

- Copilot ACP を TCP mode で起動する長寿命の control plane を立ち上げる
- Axum backend + Leptos CSR frontend の web surface を別 Deployment として提供する
- Copilot / GitHub / SSH client state を PVC に残し、Pod 再作成後も再開できるようにする
...
- browser からは HTTP API / UI だけを見せ、ACP の詳細は backend 内で閉じ込められる
...
workspace PVC は sample 既定で `ReadWriteOnce` ですが、Execution Pod は
...
から同じ full commit SHA tag を選び、Deployment / bootstrap / job transfer の
3 箇所をそろえてください。
...
sample では browser / API 向けの入口は `control-plane-web` Service です。
...
別 terminal から health check:

```bash
curl http://127.0.0.1:8080/healthz
```

そのあと、browser で `http://127.0.0.1:8080/` を開いて UI を確認します。

内部 ACP port を spot check したい場合だけ、別 terminal で次も使えます。
...
必要なら別 terminal で ACP service も見られます。
...
### current-cluster の spot check を取る

```bash
kubectl get pods,svc -n copilot-sandbox
kubectl logs deployment/control-plane -n copilot-sandbox
kubectl logs deployment/control-plane-web -n copilot-sandbox
```
