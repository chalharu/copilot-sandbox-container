...
Control Plane を提供します。起動時に Copilot / gh / session state、
`runtime.env`、bundled hook / skill、`control-plane-run` を整え、sample manifest
では main Deployment が Copilot ACP runtime を動かし、別 Deployment の web UI から
browser 経由で使えるようにします。

## このコンテナイメージが行うこと

- Copilot ACP runtime と browser-facing web UI を分離して起動する
- Copilot / GitHub / session state を PVC に残し、Pod 再作成後も再開できるようにする
...
- `kubectl exec` に依存せず、browser から対話面へ入れる
...
から同じ full commit SHA tag を選び、ACP Deployment / web Deployment /
bootstrap / job transfer をそろえてください。
...
別 terminal から browser または health endpoint を確認します:
...
`kubectl` 到達性を前提に、runtime / legacy SSH compatibility / Job path を
まとめて確認します。
...
`control-plane-acp` です。通常は browser 側だけ見れば十分です。
