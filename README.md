...
- `scripts/test-k8s-job.sh`: 既存 cluster 上の least-privilege Job smoke test
...
control-plane container は `seccompProfile: Unconfined` と `appArmorProfile.type: Unconfined` を前提にしています。default networking を使う local Podman は `/dev/net/tun` も必要です。`control-plane-run --mode k8s-job` または
