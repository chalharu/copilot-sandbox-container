...
`SETGID` / `SETUID` に絞っています。`allowPrivilegeEscalation` は `sshd` の
setuid/setgid と entrypoint の root 操作のため `true` のままですが、
...
`SETGID` / `SETUID` に絞り、`RuntimeDefault` seccomp と
`CONTROL_PLANE_RUN_MODE=k8s-job` を使っているため、SSH で入ったあとも権限を
