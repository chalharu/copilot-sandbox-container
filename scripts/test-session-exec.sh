...
"${container_bin}" run --rm --user 0:0 \
  ...
  -w /workspace \
  "${rust_test_image}" \
  bash -lc "export PATH=/usr/local/cargo/bin:\$PATH CARGO_TARGET_DIR=/tmp/control-plane-rust-target/runtime-tools-test && cargo test --manifest-path containers/control-plane/runtime-tools/Cargo.toml session_exec_command"
