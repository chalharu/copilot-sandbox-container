...
test "$(jq -r '.spec.containers[0].volumeMounts[] | select(.mountPath == "/environment/root/root/.config/gh").readOnly' /workspace/k8s-fast-exec-pod.json)" = "false"
test "$(jq -r '.spec.containers[0].volumeMounts[] | select(.mountPath == "/environment/root/root/.ssh").readOnly' /workspace/k8s-fast-exec-pod.json)" = "true"
